...
export const selectChatUser = createAsyncThunk("select-chat-user", async (id, { dispatch }) => {
  const response = await axios.get(`http://localhost:5050/api/v1/user/get-chat?id=${id}`)
...
export const getAllChat = createAsyncThunk("get-all-contacts-and-chats", async () => {
  const response = await axios.get("http://localhost:5050/api/v1/user/get-all-contacts-and-chats")
  const data = {
    profileUser: {
      id: "admin",
